...
export class DevelopersSearchComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.searchControl.valueChanges
      .pipe(
        debounceTime(300),
        switchMap((term) => this.getDevelopers(term ?? undefined)) // null → undefined
      )
      .subscribe({
  ...
  getProfileImage(path?: string | null): string {
    ...
    // Normalize backend base (no trailing slash)
    const base = environment.apiUrl.replace(/\/+$/, '');

    // If path starts with '/', just append to base
    if (path.startsWith('/')) return `${base}${path}`;

    // Bare relative like "profile-images/xxx.png"
    return `${base}/${path}`;
  }

  // Fallback if the URL 404s
