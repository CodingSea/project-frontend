...
export class IssuePageTemplate implements AfterViewInit, OnInit
{
    ...
    constructor(private issueService: IssueService, private route: ActivatedRoute, private http: HttpClient)
    {
    ...
    async getIssueId()
    {
        ...
        {
            await this.http.get<Issue>(`${environment.apiUrl}/service/${this.route.snapshot.paramMap.get('serviceId')}/issue`).subscribe(
                (res) =>
                {
                    id = res.id!;
