...
export class IssuePageTemplate implements AfterViewInit, OnInit
{
  ...
  fixImage(path: string | null | undefined, userId?: number): string
  {
    ...
    // 1. If no path, return an absolute path to your local asset
    if (!path || path === 'images/user.png') return '/assets/images/user.png';

    // 2. If it's already a full URL (from S3), return it
    if (path.startsWith('http')) return path;

    // 3. Build local backend URL safely
    const baseUrl = environment.apiUrl.replace('/api', '');
    const url = `${baseUrl}/${path}`.replace(/(?<!:)\/+/g, '/'); // Safe regex
